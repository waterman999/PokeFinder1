...
WildSearcher5::WildSearcher5(const Profile5 &profile, Method method) : profile(profile), method(method), searching(false), progress(0)
{
}

void WildSearcher5::startSearch(const WildGenerator5 &generator, int threads, Date start, const Date &end)
{
    searching = true;
...
void WildSearcher5::cancelSearch()
{
    searching = false;
}

std::vector<SearcherState5<WildState>> WildSearcher5::getResults()
{
...
int WildSearcher5::getProgress() const
{
    return progress;
}

void WildSearcher5::search(WildGenerator5 generator, const Date &start, const Date &end)
{
                ...
                for (u8 hour = 0; hour < 24; hour++)
                {
                    for (u8 minute = 0; minute < 60; minute++)
                    {
                        for (u8 second = 0; second < 60; second++)
                        {
                            ...
                            if (!states.empty())
                            {
                                std::vector<SearcherState5<WildState>> displayStates;
